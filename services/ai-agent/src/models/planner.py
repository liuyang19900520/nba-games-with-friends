...
class Plan(BaseModel):
    """一个包含分析思路和具体执行步骤的计划。"""
    reasoning: str = Field(description="在制定计划前的思考过程。分析用户意图，确定需要哪些数据。")
    steps: List[str] = Field(description="具体的执行步骤列表，每一步都应该是清晰、独立的行动。")
...
class Act(BaseModel):
    ...
    """
    response: Union[str, List[str]] = Field(description="如果任务完成，返回最终答案字符串；如果任务未完成，返回新的步骤列表。")
